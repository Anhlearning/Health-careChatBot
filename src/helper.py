...
# Import local files
from .prompt import prompt_template  # <-- sửa ở đây
from config import *
...
def initialize_llm(user_name, host, k=2, max_tokens=512, temp=0.1):
    ...
    retriever_tool = create_retriever_tool(retriever, name="search_medical_information",
                                           description="Use to look up additional medical context and information to answer the question.")
    ...
    search_doctor_by_specialization_tool = StructuredTool.from_function(
        func=search_doctor_by_specialization, name="search_doctor_by_specialization",
        description="Use to look up a list of doctor with a desired specialization.", handle_tool_error=True)

    search_available_doctor_appointments_tool = StructuredTool.from_function(
        func=search_available_doctor_appointments, name="search_available_doctor_appointments",
        description="Use to look up a list of available time slots for appointments with a given doctor.", handle_tool_error=True)

    search_patient_appointments_tool = StructuredTool.from_function(
        func=search_patient_appointments, name="search_patient_appointments",
        description="Use to look up the list of appointments currently scheduled by the patient", handle_tool_error=True)

    register_emergency_tool = StructuredTool.from_function(
        func=register_emergency, name="register_emergency",
        description="Use to register a medical emergency manifested by a patient with a corresponding color-code.", handle_tool_error=True)

    tools += [retriever_tool, search_doctor_by_specialization_tool, search_available_doctor_appointments_tool,
              search_patient_appointments_tool, register_emergency_tool]

    def state_modifier(state):
        prompt = SystemMessage(content=prompt_template.format(user_name=user_name,
                                                              table_names=db.get_usable_table_names(), host=host))
